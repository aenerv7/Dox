...
The command runs the Vitest suite, TypeScript checks, the Vite production build, and `web-ext build`. The submitted archive is reproduced at `web-ext-artifacts/dox_reader-0.1.0.zip`. Dependencies are resolved only through the public npm registry using `package-lock.json`.

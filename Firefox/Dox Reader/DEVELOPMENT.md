...
| 0.3.2 | 修复文章标题中的特殊字符未解码问题：fast-xml-parser 默认不解码数字字符引用（如 WordPress 输出的 `&#038;`，即 `&`）与常见 HTML 命名实体（`&rsquo;`、`&mdash;` 等），现于解析与已存数据迁移两处补齐解码（`decodeHtmlEntities`）；guid/文章 id 保持原始文本以免产生重复条目 |

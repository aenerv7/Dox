...
The command runs the Vitest suite, TypeScript checks, the Vite production build, and `web-ext build`. The submitted archive is reproduced under `web-ext-artifacts/`. Dependencies are resolved only through the public npm registry using `package-lock.json`.
